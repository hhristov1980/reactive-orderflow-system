...
@Slf4j
public class InventoryOrderCancelledConsumer {
    ...
    public void consumeOrderCancelled(String payload) {
        ...
        inventoryService.release(event)
                .flatMap(inventoryEventProducer::publishInventoryReleased)
                .doOnError(error ->
                        log.error(
    ...
    private <T> T readEvent(
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Failed to deserialize order.cancelled payload: " + payload,
                    ex
            );
        }
    }
